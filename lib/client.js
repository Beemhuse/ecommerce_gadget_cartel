...
export const updateTransactionStatus = async (trxref, newStatus) => {
  try {
    // Fetch the transaction document from Sanity
    const existingTransaction = await client.getDocument(trxref);

    if (!existingTransaction) {
      throw new Error(`Transaction with ID ${trxref} not found`);
    }
