...
export const verifyPaystackPayment = async (txref) => {
  try {
    const paystackApiUrl = `https://api.paystack.co/transaction/verify/${txref}`;
    const response = await axios.get(paystackApiUrl, {
      headers: {
        Authorization: `Bearer ${process.env.NEXT_PUBLIC_PAYSTACK_SECRET_KEY}`,
      },
    ...
    const responseData = response.data;
    console.log('Paystack Verification Response:', txref, response.data);
